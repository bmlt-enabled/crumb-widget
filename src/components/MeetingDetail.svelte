...
<div class="flex h-full flex-col">
  ...
  <div class="shrink-0 border-b border-gray-200 bg-white px-4 py-3">
    <button onclick={clearSelectedMeeting} class="bmlt-link mb-1.5 flex cursor-pointer items-center gap-1 text-sm text-blue-600 hover:text-blue-800">
      <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
      </svg>
      {$t.backToMeetings}
    </button>
    <h2 class="text-xl font-bold text-gray-900">{meeting.meeting_name}</h2>
  </div>
    ...
    <div class="shrink-0 overflow-y-auto p-4 sm:w-72 sm:border-r sm:border-gray-200 lg:w-80">
      ...
      <div class="bmlt-card divide-y divide-gray-100 rounded-lg border border-gray-200">
        ...
        <div class="bg-gray-50 px-4 py-2.5 first:rounded-t-lg">
          <p class="text-sm font-semibold text-gray-800">Meeting Information</p>
        </div>

        <!-- Schedule row -->
        <div class="px-4 py-3">
          <p class="text-xs font-semibold tracking-wide text-gray-400 uppercase">{$t.schedule}</p>
          ...
          {#if meeting.duration_time && meeting.duration_time !== '01:00:00'}
            <p class="mt-0.5 text-xs text-gray-500">{$t.duration}: {meeting.duration_time.slice(0, 5).replace(/^0/, '')}</p>
          {/if}
        ...
        <!-- Venue + formats row -->
        <div class="px-4 py-3">
          <p class="text-xs font-semibold tracking-wide text-gray-400 uppercase">{$t.venueType}</p>
          <div class="mt-1 flex flex-wrap gap-1.5">
            {#if meeting.isInPerson}
              <span class="bmlt-badge-in-person inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700">{$t.inPerson}</span>
            {/if}
            {#if meeting.isVirtual}
              <span class="bmlt-badge-virtual inline-flex items-center rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-700">{$t.virtual}</span>
            {/if}
          </div>
          {#if meeting.resolvedFormats.length > 0}
            <div class="mt-2 flex flex-wrap gap-1">
              {#each meeting.resolvedFormats as fmt (fmt.id)}
                <span class="rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-600" title={fmt.description_string}>{fmt.name_string}</span>
              {/each}
            </div>
          {/if}
        </div>
        ...
        {#if meeting.isInPerson && meeting.formattedAddress}
          <div class="px-4 py-3">
            <p class="text-xs font-semibold tracking-wide text-gray-400 uppercase">{$t.address}</p>
            {#if meeting.location_text}
              <p class="mt-1 text-sm font-medium text-gray-900">{meeting.location_text}</p>
            {/if}
            <p class="mt-0.5 text-sm text-gray-700">{meeting.formattedAddress}</p>
            {#if meeting.location_info}
              <p class="mt-0.5 text-xs text-gray-500">{meeting.location_info}</p>
            {/if}
          </div>
        {/if}
        ...
        {#if meeting.isVirtual && (meeting.virtual_meeting_link || meeting.virtual_meeting_additional_info)}
          <div class="px-4 py-3">
            <p class="text-xs font-semibold tracking-wide text-gray-400 uppercase">{$t.onlineMeetingSection}</p>
            {#if meeting.virtual_meeting_link}
              <a
                href={meeting.virtual_meeting_link}
                target="_blank"
                rel="noopener noreferrer"
                class="bmlt-btn-primary mt-1.5 flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-all hover:brightness-90"
              >
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
            ...
            {#if meeting.virtual_meeting_additional_info}
              <p class="mt-1.5 text-xs text-gray-600">{meeting.virtual_meeting_additional_info}</p>
            {/if}
          </div>
        {/if}
        ...
        {#if meeting.comments}
          <div class="px-4 py-3">
            <p class="text-xs font-semibold tracking-wide text-gray-400 uppercase">{$t.notes}</p>
            <p class="mt-1 text-sm text-gray-600">{meeting.comments}</p>
          </div>
        {/if}
        ...
        {#if meeting.service_body_name}
          <div class="px-4 py-3">
            <p class="text-xs font-semibold tracking-wide text-gray-400 uppercase">{$t.serviceBody}</p>
            <p class="mt-1 text-sm text-gray-700">{meeting.service_body_name}</p>
          </div>
        {/if}
        ...
        {#if meeting.email_contact}
          <div class="px-4 py-3">
            <p class="text-xs font-semibold tracking-wide text-gray-400 uppercase">{$t.contact}</p>
            <a href="mailto:{meeting.email_contact}" class="bmlt-link mt-1 block text-sm text-blue-600 hover:text-blue-800">
