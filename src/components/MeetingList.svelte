  ...
  {#if meeting.resolvedFormats.length > 0}
    <div class="flex flex-wrap gap-1">
      {#each meeting.resolvedFormats as fmt (fmt.id)}
        <span class="rounded border border-gray-200 bg-white px-2 py-0.5 text-xs text-gray-700" title="{fmt.name_string} — {fmt.description_string}">{fmt.name_string}</span>
      {/each}
    </div>
  {/if}
{/snippet}
