<script lang="ts">
  ...
  import type { ProcessedMeeting, MarkerConfig } from '@/types/index';
  ...
  const { meetings, locationMarker }: Props = $props();

  let mapEl: HTMLDivElement;
  let leafletMap: LeafletMap | null = null;
  let markersLayer: LayerGroup | null = null;

  const DEFAULT_LOCATION_MARKER: MarkerConfig = {
  ...
  function buildIcon(cfg: MarkerConfig): L.DivIcon {
    return L.divIcon({
      html: cfg.html,
      iconSize: new L.Point(cfg.width, cfg.height),
      iconAnchor: [Math.floor(cfg.width / 2), cfg.height],
      popupAnchor: [0, -cfg.height],
      className: 'bmlt-marker'
    });
  }
  ...
  function renderMarkers(): void {
    ...
    for (const [key, group] of Object.entries(groups)) {
      ...
      const marker = L.marker([lat, lng], { icon: buildIcon(locationMarker ?? DEFAULT_LOCATION_MARKER) });
