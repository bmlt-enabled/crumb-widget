...
docs/
  index.html                 # Documentation page (copied to dist/index.html on build)
  meetings.html              # Demo page (copied to dist/meetings.html on build) — reads ?lang= and forwards to widget
  intl/                      # Per-language docs translations (JSON dictionaries copied to dist/intl/) and GitHub README translations (README.{lang}.md, not shipped to dist)
dist/
...
`docs/index.html` is a single template tagged with `data-i18n="key.path"` (textContent) and `data-i18n-html="key.path"` (innerHTML, used for strings containing `<a>`, `<code>`, `<strong>`, etc.). Translation dictionaries live in `docs/intl/{lang}.json`.

- Inline loader in `index.html` resolves the language as `?lang=` → `localStorage('bmlt-docs-lang')` → `navigator.language` → `en`.
- Sidebar + mobile bar both have a `<select>` switcher (`#lang-select`, `#lang-select-mobile`). Adding language #N: drop in `docs/intl/{code}.json` and add `{ code, label }` to `SUPPORTED_LANGS` in `index.html`. The `vite.config.ts` `copy-docs` plugin mirrors `docs/intl/` into `dist/intl/`, filtering out `*.md` (GitHub README translations live alongside but are not shipped).
- All `<a href="meetings.html">` links inside `index.html` get rewritten on language change to append `?lang={code}`. `meetings.html` reads that param and merges `language: <code>` into `CrumbWidgetConfig` so the embedded widget matches the docs language.
...
The README has separate translations under `docs/intl/README.{lang}.md` (these are GitHub-rendered Markdown, unrelated to the docs-page JSON dictionaries — and excluded from the `dist/` copy by the `copy-docs` filter). Keep both sets in sync when adding a new language: add the README translation and the docs-page `{lang}.json` dictionary in the same `docs/intl/` directory. The repo-root `README.md` is a symlink to `docs/intl/README.en.md`.
